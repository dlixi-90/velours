...
export const buildOrderConfirmationEmail = (order, {
  storefrontUrl = process.env.FRONTEND_URL || "https://velours-jet.vercel.app",
} = {}) => {
  ...
  const date = Number.isNaN(createdAt.getTime()) ? "" : new Intl.DateTimeFormat("vi-VN", {
    dateStyle: "long", timeZone: "Asia/Ho_Chi_Minh",
  }).format(createdAt);
  ...
  const paymentMethod = order.paymentMethod === "COD" ? "Thanh toán khi nhận hàng (COD)" : "Chuyển khoản QR";
...
<body style="margin:0;padding:0;width:100%;background:#f1f4ef;font-family:Arial,Helvetica,sans-serif;color:#263b4a;">
      ...
      <table role="presentation" width="100%" style="width:100%;max-width:600px;background:#ffffff;border:1px solid #e2e7df;">
        <tr><td class="content-pad" style="padding:28px 36px;background:#41334e;">
          <table role="presentation" width="100%"><tr>
            ...
            <td align="right" style="font-size:10px;line-height:16px;letter-spacing:2px;color:#d4cddd;">CHĂM SÓC BẠN<br>MỖI NGÀY</td>
          </tr></table>
        </td></tr>
        <tr><td class="content-pad" style="padding:34px 36px 28px;background:#f6f9f6;">
          ...
          <h1 class="headline" style="margin:0 0 16px;font:34px/43px Georgia,'Times New Roman',serif;color:#41334e;">Cảm ơn bạn đã<br>chọn Velours.</h1>
  ...
  const text = [
    ...
    "", "SẢN PHẨM",
    ...items.map((item) => `${item.title} | ${item.size} | ${money(item.unitPrice)} × ${item.quantity} = ${money(item.unitPrice * item.quantity)}`),
    "", `Tạm tính: ${money(subtotal)}`,
    `Phí vận chuyển: ${shipping === 0 ? "Miễn phí" : money(shipping)}`,
    `${totalLabel}: ${money(total)}`,
    `${paymentMethod} — ${paymentStatus}`, paymentNote,
    "", "ĐỊA CHỈ NHẬN HÀNG", recipient, ...addressLines,
